...
import { WorkflowExecutionService } from '../src/workflows/workflow-execution.service';

// This script evaluates representative If configurations without persisting any workflow runs or DB data.
// It writes results to backend/if_test.csv

async function main() {
  const prisma = new PrismaService();
  // Minimal stubs where possible
  const workflowsService = new WorkflowsService(prisma as any);
  const notifications = new NotificationsService();
  const exec = new WorkflowExecutionService(prisma as any, workflowsService as any, notifications as any);

  // Synthetic test user id for context scoping only
  const userId = 'test-user';

  // Synthetic payloads (do not persist)
  const booking = { email: 'heine@example.com', title: 'Strategy Call', eventType: 'Strategy Call' } as any;
  const contact = { email: 'contact@example.com', name: 'Jon Doe', tags: ['VIP', 'Newsletter'] } as any;
  ...
  // 1) Email contains/not_contains across many samples
  const emails: string[] = [];
  // Generate 40 diverse emails, ~half containing "heine" in various positions/cases
  for (let i = 0; i < 40; i++) {
    if (i % 2 === 0) emails.push(`heine${i}@example.com`);
    else if (i % 5 === 0) emails.push(`USER${i}@HEINE.org`);
    else if (i % 3 === 0) emails.push(`user.${i}@sample.org`);
    else emails.push(`contact${i}@company.dev`);
  }
  emails.forEach((em, i) => {
    ...
    // contact fallback path if no contact provided
  ...
  // 2) exists / not_exists on present/missing fields
  ...
  // 3) equals / not_equals numeric with coercion
  const durations = Array.from({ length: 20 }, (_ , idx) => idx * 10); // 0,10,...,190
  ...
  // 5) in list / not in list with string and array values
  ...
  // 6) equals with array actual (membership semantics)
  ...
  // 7) Error scenarios defaulting to false (missingDataBehavior)
  ...
  for (const tc of cases) {
    const props: any = { field: tc.field, operator: tc.operator, value: tc.value };
    // For error tests, opt into default false on errors
    if (tc.name.includes('error')) props.missingDataBehavior = 'default_false_path_on_error';
  ...
  const outPath = join(__dirname, '..', 'if_test.csv');
